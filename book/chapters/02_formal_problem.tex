...
\emph{earlier} event wins (since we negate the timestamp); and if both
...
Consider a dinner-party simulation~\citep{park2023generative} with $|A| = 6$ agents (Alice, Bob,
...
\begin{definition}[Bounded-prefix phase grammar]\label{def:bounded-phase-grammar}
The \textbf{bounded-prefix phase grammar} is the phase grammar of
\cref{def:phase-grammar} augmented with an upper bound $k_{\max} \ge k_{\min}$
on the \textsc{Development} phase: a run is accepted only if the number of
\textsc{Development} symbols consumed before \textsc{Turning\_Point} lies
in the closed interval $[k_{\min}, k_{\max}]$.  The lower bound $k_{\min}$
is the standard prefix requirement~$k$ of \cref{def:phase-grammar}; the
upper bound $k_{\max}$ prevents excessively long setup portions of the
narrative arc.
\end{definition}
...
not a greedoid~\citep{korte1981mathematical,bjorner1992greedoids}, and not a polymatroid.
\end{proposition}
...
\smallskip
We use the bounded-prefix phase grammar (\cref{def:bounded-phase-grammar})
with $k_{\min} = 3$ and $k_{\max} = 4$: at least~3 and at most~4
\textsc{Development} symbols must appear before the turning point.
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Adding $q$} (focal, weight~9, timestamp~$3.5$) to~$S_1$.
    The augmented set is
    $S_1 \cup \{q\} = \{c_1, c_3, q, c_4,$ $c_5, p, r\}$.
    Now $\tp(S_1 \cup \{q\}) = q$ because
    $w(q) = 9 > 7 = w(p)$---the pivot shifts from~$p$ to~$q$.
    Events before $q$ (timestamp~$3.5$):
...
\begin{remark}[Bounded-prefix scope]\label{rem:exchange-scope}
The exchange-axiom failure above is specific to the bounded-prefix
grammar $\dpre \ge k$.  Different grammars (e.g., a flat ``select
any $m$ events'' objective) may or may not exhibit a matroid
structure; the non-matroid conclusion does not extend beyond the
class of grammars with positional requirements relative to the pivot.
\end{remark}
