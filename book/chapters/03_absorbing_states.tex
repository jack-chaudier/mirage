...
The three conditions in the definition deserve emphasis.
Condition~(i), $v \neq e^{*}$, excludes the turning point itself.
Condition~(ii), $t(v) < t(e^{*})$, restricts attention to events that
...
(\cref{lem:dfa-monotonicity}).  Condition~(iii) encodes the
...
We summarise the main findings; full experimental details appear in the
repository.
...
$e^{*} = \argmax_{v : a(v)=a^{*}} w(v)$, we assign the turning point
uniformly at random.
