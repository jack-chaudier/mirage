...
\subsection{Viability-Aware Greedy (VAG) Algorithms}\label{subsec:vag}

The VAG family of algorithms shares a common skeleton: greedily select
events in weight-descending order, but reject any candidate whose
addition would render a future valid completion impossible.  The variants
differ only in which viability checks are applied.
\Cref{alg:vag} presents the common framework;
\cref{alg:viable} details the viability predicate with the three
filtering levels.

\begin{algorithm}[t]
\caption{Viability-Aware Greedy Extraction (VAG)}
\label{alg:vag}
\begin{algorithmic}[1]
\Require Pool $P$, anchor $e_0$, grammar $\mathcal{A}$ with prefix
         requirement~$k$, max length $n_{\max}$, viability mode
         $\textsc{mode} \in \{\textsc{span}, \textsc{gap}, \textsc{budget}\}$
\Ensure Selected event sequence $S$ (sorted by timestamp)
\Statex
\Function{VAG\_Extract}{$P,\, e_0,\, \mathcal{A},\, n_{\max},\, \textsc{mode}$}
  \State $S \gets \{e_0\}$
         \Comment{Seed with anchor event}
  \State $R \gets \Call{SortByWeight}{P \setminus \{e_0\}}$
         \Comment{Remaining candidates, weight-descending}
  \While{$|S| < n_{\max}$ \textbf{and} $R \neq \varnothing$}
    \State $\mathit{accepted} \gets \textsc{false}$
    \For{each $c \in R$ in weight order}
      \If{$\Call{IsViable}{S,\, c,\, R \setminus \{c\},\, \mathcal{A},\, n_{\max},\, \textsc{mode}}$}
        \State $S \gets S \cup \{c\}$;\quad
               $R \gets R \setminus \{c\}$
        \State $\mathit{accepted} \gets \textsc{true}$;\quad
               \textbf{break}
      \EndIf
    \EndFor
    \If{$\lnot\,\mathit{accepted}$}
      \State \textbf{break}
           \Comment{No viable candidate remains}
    \EndIf
  \EndWhile
  \State \Return $\Call{SortByTimestamp}{S}$
\EndFunction
\end{algorithmic}
\end{algorithm}

\begin{algorithm}[t]
\caption{Viability Predicate (\textsc{IsViable})}
\label{alg:viable}
\begin{algorithmic}[1]
\Require Current selection $S$, candidate $c$, remaining pool $R$,
         grammar $\mathcal{A}$, max length $n_{\max}$,
         mode $\in \{\textsc{span}, \textsc{gap}, \textsc{budget}\}$
\Ensure \textsc{true} if adding $c$ preserves feasibility
\Statex
\Function{IsViable}{$S,\, c,\, R,\, \mathcal{A},\, n_{\max},\, \textsc{mode}$}
  \State $S' \gets S \cup \{c\}$;\quad
         $r \gets n_{\max} - |S'|$
         \Comment{Remaining slots}
  \Statex \Comment{\textbf{--- Common checks (all modes) ---}}
  \If{$|S'| > n_{\max}$}
    \Return \textsc{false}
  ...
  \If{$|S'| + \min(r, |R|) < n_{\min}$}
    \Return \textsc{false}
    \Comment{Cannot reach min length}
  \EndIf
  \If{$\Call{MaxCoverage}{S', R} < \rho$}
    \Return \textsc{false}
    \Comment{Focal-actor coverage unreachable}
  \EndIf
  \If{$\Call{MaxSpan}{S', R, r} < \gamma \cdot T$}
    \Return \textsc{false}
    \Comment{Span unreachable}
  \EndIf
  \If{$\Call{TPLocked}{S', R, \mathcal{A}}$}
    \Return \textsc{false}
    \Comment{TP locked with $< k$ dev events}
  \EndIf
  \Statex \Comment{\textbf{--- Gap-aware check (modes: gap, budget) ---}}
  \If{$\textsc{mode} \in \{\textsc{gap}, \textsc{budget}\}$}
    \For{each oversized gap $(t_l, t_r)$ in $S'$ with $t_r - t_l > G_{\max}$}
      \If{no bridge event exists in $R$ with $t_l < t(v) < t_r$}
        \Return \textsc{false}
        \Comment{Gap unbridgeable}
      \EndIf
    \EndFor
  \EndIf
  \Statex \Comment{\textbf{--- Budget-aware check (mode: budget) ---}}
  \If{$\textsc{mode} = \textsc{budget}$}
    \State $B_{\mathrm{lb}} \gets \sum_{(t_i, t_{i+1}) \in S'}
           \max\!\bigl(0,\, \lceil (t_{i+1} - t_i) / G_{\max} \rceil - 1\bigr)$
    \If{$B_{\mathrm{lb}} > r$}
      \Return \textsc{false}
      \Comment{Bridge budget exceeded}
    \EndIf
  \EndIf
...
The three modes correspond to the hierarchy levels:
\textbf{Span-VAG} (Level~1) uses only the common checks,
\textbf{Gap-VAG} (Level~2) adds the gap-bridgeability check, and
\textbf{BVAG} (Level~2.5) further adds the bridge-budget reservation
that ensures the remaining slot count can accommodate the minimum number
of bridge events implied by oversized gaps.
...
$\argmax$ pivot~\citep{gange2020argmax}, the TP-Solver considers the top-$M$ focal events by
...
path problem~\citep{irnich2005shortest} (RCSPP), which the label-setting DP solves efficiently with
