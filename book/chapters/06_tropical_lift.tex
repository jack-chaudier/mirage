...
shift-and-max rule:
\begin{enumerate}[label=(\roman*)]
  \item Initialise $W_{\mathrm{result}} := W_A$ (copy all of $A$'s entries).
  \item For each $j' = 0, 1, \ldots, k$, let $j = \min(j' + d_A,\; k)$
    and set
    \begin{equation}\label{eq:shift-and-max}
      W_{\mathrm{result}}[j]
      \;\leftarrow\;
      \max\!\bigl(W_{\mathrm{result}}[j],\; W_B[j']\bigr).
    \end{equation}
\end{enumerate}
\end{definition}

The logic is as follows.  A pivot from block $A$ that already had $j$
pre-pivot development events keeps its slot: step~(i) copies all of
$A$'s entries into the result.  A pivot from block $B$ that had $j'$
pre-pivot development events within $B$ acquires all $d_A$ of $A$'s
non-focal events as additional predecessors, so step~(ii) writes it to
result slot $\min(j' + d_A,\; k)$.  When $j' + d_A > k$, the shifted
slot is capped at $k$ (both slots $j' + d_A$ and $k$ indicate ``at
least $k$ pre-pivot events''), and the $\max$ accumulates multiple
$B$-pivots that map to the same capped slot.
...
The iteration in \cref{eq:shift-and-max} handles capping explicitly:
a pivot from $B$ at slot $j'$ with $j' + d_A > k$ is written to
result slot $\min(j' + d_A,\; k) = k$, accumulating via $\max$ with
any value already at slot~$k$.  This ensures that pivots with ``at
least $k$'' pre-pivot events are correctly aggregated.
\end{remark}
...
\end{align*}
We compute $T_A \otimes T_B$ using the iteration form.
Initialise $W_{\mathrm{result}} = W_A = [5.0,\; 3.0,\; -\infty,\; -\infty]$.
\begin{align*}
  j' = 0 &\colon\; j = \min(0+2,\;3) = 2.\quad
    W_{\mathrm{result}}[2] \leftarrow \max(-\infty,\; W_B[0])
    = \max(-\infty,\; 7.0) = 7.0. \\[3pt]
  j' = 1 &\colon\; j = \min(1+2,\;3) = 3.\quad
    W_{\mathrm{result}}[3] \leftarrow \max(-\infty,\; W_B[1])
    = \max(-\infty,\; -\infty) = -\infty. \\[3pt]
  j' = 2 &\colon\; j = \min(2+2,\;3) = 3.\quad
    W_{\mathrm{result}}[3] \leftarrow \max(-\infty,\; W_B[2])
    = \max(-\infty,\; 4.0) = 4.0. \\[3pt]
  j' = 3 &\colon\; j = \min(3+2,\;3) = 3.\quad
    W_{\mathrm{result}}[3] \leftarrow \max(4.0,\; W_B[3])
    = \max(4.0,\; -\infty) = 4.0.
\end{align*}
The result is
\[
  T_A \otimes T_B
  = \bigl(\,[5.0,\; 3.0,\; 7.0,\; 4.0],\;\; d = 5\bigr).
\]
...
$d_A = 2$).  Slot~3 has weight $4.0$: this comes from $W_B[2]$,
which maps to slot $\min(2+2,\;3) = 3$ via capping.  The context is
\textbf{feasible} at $k = 3$ with best feasible pivot weight $4.0$.
\end{example}
