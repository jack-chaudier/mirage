...
ultimately serve as the pivot, nor which events will appear in the
subsequent suffix.
...
\begin{remark}[Necessary vs.\ sufficient guards]\label{rem:necessary-sufficient}
\Cref{def:no-absorption-contract} states a \emph{necessary} condition
for no absorption: if $\dtotal(\mu(B)) < \min(\dtotal(B),\, k)$,
absorption is guaranteed.  The algorithm's tropical $W[k]$ guard is a
\emph{sufficient} condition: it checks whether the best feasible
pivot---one with at least $k$ non-focal predecessors---survives compression.
The $W[k]$ guard is strictly stronger than the $\dtotal$ contract:
one can satisfy $\dtotal \ge k$ while placing all $k$ non-focal events
\emph{after} every focal event, which destroys the pivot's pre-pivot
prefix.  The $W[k]$ guard catches this case; the $\dtotal$ contract does not.
In practice, use the $W[k]$ guard when tropical contexts are available
and fall back to the $\dtotal$ contract when only event counts are known.
\end{remark}
