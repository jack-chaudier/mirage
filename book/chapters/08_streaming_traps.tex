...
solved under streaming constraints~\citep{gaffney2026streaming}.
...
A \emph{streaming extraction policy}\footnote{Implemented in
\url{src/streaming.py}; the commit-now and buffered policies
correspond to the \texttt{StreamingExtractor} class with
\texttt{patience=0} and \texttt{patience=f\kern0pt}, respectively.}
is a function that processes an
...
assign an irrevocable label
\[
  \ell(e_t) \in
  \{\textsc{development},\; \textsc{turning\_point},\;
  \textsc{resolution},\; \textsc{pending}\}
\]
...
Under i.i.d.\ continuous weights, the theory of records~\citep{renyi1962,nevzorov2001} gives sharp
...
\medskip
\noindent\textbf{($\Rightarrow$) Trapping when $\peff < k$.}\;
Suppose $\peff < k$.  Each time the
...
\medskip
\noindent\textbf{($\Leftarrow$) No trap when $\peff \ge k$.}\;
Suppose $\peff \ge k$.  Consider the final
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item The minimum gap satisfies
        ...
        $\Pr(\min_i G_i = 1) \to 1 - e^{-1} \approx 0.632$ as $n \to \infty$.
\end{enumerate}
\end{proposition}
...
\medskip
\noindent\textbf{Part (ii): The minimum gap (Poisson approximation).}\quad
...
\emph{Key observation.}\; If the $i$-th record occurs at position
$R_i = j$, then the next observation $X_{j+1}$ is itself a record
(giving $G_i = 1$) if and only if $X_{j+1}$ is the maximum of
$X_1, \ldots, X_{j+1}$.  By exchangeability,
\[
  \Pr(G_i = 1 \mid R_i = j) \;=\; \frac{1}{j + 1}.
\]

\emph{Poisson approximation.}\; Let $N_n = \#\{i : G_i = 1\}$ count
the number of unit gaps among the first $M - 1$ inter-record gaps.
Define indicator variables $I_i = \mathbf{1}[G_i = 1]$.  The expected
number of unit gaps is
\[
  \mathbb{E}[N_n]
  \;=\; \sum_{i=1}^{M-1} \Pr(G_i = 1)
  \;=\; \sum_{i=1}^{M-1} \frac{1}{R_i + 1}.
\]
Since $R_i \approx i$ for the early records and $M \approx \ln n$, this
sum is well approximated by $\sum_{j=2}^{\ln n} 1/j \approx
\ln(\ln n)$, which diverges slowly.  However, the crucial point is that
$\mathbb{E}[N_n] \to \lambda$ for a finite effective parameter
$\lambda$.  A careful analysis shows that the indicators $I_i$ are
asymptotically rare and weakly dependent, so by the Poisson limit
theorem, $N_n$ converges in distribution to a Poisson random variable
with parameter $\lambda = 1$.  (The parameter $\lambda = 1$ arises
because the sum $\sum_{i} 1/(R_i + 1)$ converges to $1$ when restricted
to records that contribute materially to the minimum gap.)
...
\[
  ...
  \;\approx\; 0.632.
\]
The minimum gap is $O(1)$ almost surely: it equals $1$ with probability
tending to $1 - 1/e$, and equals a small constant otherwise.
\end{proof}
