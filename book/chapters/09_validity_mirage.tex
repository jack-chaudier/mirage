...
uncompressed solve---collapses from near $1.0$ to $0.328$ over the same
...
from Paper~03~\citep{gaffney2026mirage}: $n = 200$ instances (200 events each), prefix requirement
...
\caption{The validity mirage across retention levels ($n = 200$
         instances, $k = 3$, $M = 10$).  Raw validity stays at or near $1.0$
         across all retention levels, while pivot preservation collapses
         from $0.785$ at 50\% retention to $0.328$ at 10\% retention.
         The mirage gap---the difference between raw validity and pivot
         preservation---widens to ${\approx}\,0.647$ at 10\% retention.}
...
\midrule
0.50 & 1.000 & 0.785        & 0.980        & 0.281    \\
0.45 & 1.000 & 0.745        & 0.970        & 0.299    \\
0.40 & 1.000 & 0.705        & 0.970        & 0.365    \\
0.35 & 1.000 & 0.640        & 0.960        & 0.559    \\
0.30 & 1.000 & 0.595        & 0.930        & 0.650    \\
0.25 & 1.000 & 0.545        & 0.905        & 0.706    \\
0.20 & 1.000 & 0.510        & 0.875        & 0.643    \\
0.15 & 0.995 & 0.392        & 0.825        & 0.530    \\
0.10 & 0.975 & 0.328        & 0.725        & 0.680    \\
\bottomrule
\end{tabular}
\end{table}
...
The first column is monotonically near-perfect.  At 50\% retention, all
200 instances produce valid output.  At 30\%, still all 200.  At 15\%,
$99.5\%$ remain valid.  Even at 10\% retention---where 90\% of the
non-focal events have been removed---validity drops only to $0.975$.  A
...
The second column tells a different story.  By 50\% retention,
only 78.5\% of valid outputs keep the same pivot---one in five has
silently substituted.  By 30\%, only 59.5\% preserve the original; by
10\%, only 32.8\%.  At 10\% retention, the system substitutes the
...
original pivot---so the 21.5\% of outputs that substituted pivots
\emph{could have} kept the original, but the enumerative solver found
an alternative path.  By 10\% retention, only 72.5\% of instances
remain feasible under the original pivot.  The remaining 27.5\%
\emph{cannot} be solved with the original pivot at all: compression
...
quality loss among substituted instances is 28.1\%.  Regret climbs
steeply through the 40--25\% range, peaking at 70.6\% at 25\%
retention.  Below 25\%, the survivorship pool shifts: the instances
most severely harmed by compression enter infeasibility entirely and
exit the regret average, causing some fluctuation.  At 10\% retention,
average regret remains high at 68.0\%.

\begin{remark}[Non-monotonicity of semantic regret is a conditioning artifact]
\label{rem:regret-nonmonotone}
The dip in semantic regret between 25\% and 20\% retention
($0.706 \to 0.643$) is not noise: it is a consequence of how regret is
conditioned.  Regret is computed only over instances where the
fixed-pivot solve succeeds (\cref{rem:regret-streaming-caveat}).  As
retention falls, the most severely compressed instances---those where
pivot infeasibility is total---exit the conditioning set entirely.
The remaining instances are an increasingly selected sample: those
where the enumerative solver still finds \emph{some} feasible
alternative, and for which the quality loss, though large, is finite.
This survivorship bias makes measured regret appear to plateau or dip
at extreme compression even as the true harm (including the now-excluded
infeasible cases) continues to worsen.  The fixed-pivot feasibility
column (\cref{tab:retention-sweep}) is the correct lens for assessing
total harm: it does not condition on solver success and falls
monotonically from $0.980$ to $0.725$.
\end{remark}
...
gap is $0.975 - 0.328 = 0.647$---nearly two-thirds of all outputs are
mirages.
...
\begin{tikzpicture}[
  ...
  \node[subpivot] at (6*1.1, -3.6) {$e_{8}$};
  \draw[brace] (0, -3.05) -- node[above=3pt, label]
    {$\dpre \ge k$} (5*1.1, -3.05);
...
from $0.706$ at 25\% retention to $0.530$ at 15\% retention, even though
...
at 15\% retention?  What happened to the instances with the highest
potential regret?  Also explain why regret \emph{rises} again at 10\%
retention ($0.680$) relative to 15\% ($0.530$).
\end{exercise}
