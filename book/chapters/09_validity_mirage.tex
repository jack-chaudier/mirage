...
from Paper~03: $n = 200$ random event sequences, prefix requirement
...
\caption{The validity mirage across retention levels ($n = 200$,
         ...
         between raw validity and pivot preservation---widens to $0.636$
         at 10\% retention.}
...
\midrule
0.90 & 1.000 & ${\sim}1.0$  & ${\sim}1.0$  & low      \\
0.70 & 1.000 & ${\sim}0.85$ & ${\sim}0.98$ & moderate \\
...
\caption{The validity mirage: raw validity (blue) stays near $1.0$
         while pivot preservation (red) collapses under increasing
         compression.  The widening gap between the two curves is the
         mirage: the region where the system reports success but has
         silently substituted the semantic anchor.}
...
gap is $1.000 - 0.354 = 0.646$---nearly two-thirds of all outputs are
mirages.
...
\texttt{deterministic\_mirage\_witness(k=3)} in
\texttt{src/compression.py}.  It constructs a specific event sequence
designed to satisfy two properties:
\begin{enumerate}[label=(\roman*),itemsep=3pt]
...
The dominant pivot is event $e_{41}$ (or a similarly indexed late-sequence
focal event), with weight $\wstar \approx 47.08$.  Because $e_{41}$
appears late in the sequence, there are many non-focal events preceding
it: $\dpre \ge k = 3$.  The solve is \textbf{feasible}, the quality
score is $47.08$, and this is the reference solution against which all
...
$M = 1$ is required to use the original pivot $e_{41}$.  After
compression, the non-focal events that preceded $e_{41}$ have been
...
($M = 10$ pivot candidates).  The solver discovers that $e_{41}$ is
infeasible and begins searching alternatives.  It finds event $e_{45}$
(or a similarly indexed event): a focal event with lower weight
$\wstar \approx 21.47$, appearing at an earlier position in the
...
\[
  \mathrm{SR}
  \;=\;
  1 - \frac{21.467}{47.084}
  \;=\;
  0.544.
\]
The system reports a valid output.  The grammar is satisfied.  The
phase labels are well-formed.  And 54.4\% of the semantic quality has
been silently discarded.  The output tells a different story---one
anchored at a weaker, earlier event with less dramatic weight---and no
...
\begin{tikzpicture}[
  ...
  \node[subpivot] at (3*1.1, -3.6) {$e_{45}$};
...
\caption{Mechanism diagram for the deterministic witness.
         ...
         $e_{41}$ (orange, double border) late, with sufficient
...
Contract & 1  & True  & $e_{41}$& True  & True  \\
Naive    & 10 & True  & $e_{45}$& False & False \\
Contract & 10 & True  & $e_{41}$& True  & True  \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[itemsep=4pt]
  \item \textbf{Naive, $M = 1$:}  The committed solver finds the
        original pivot $e_{41}$ infeasible and has no alternatives.
  ...
  \item \textbf{Contract, $M = 1$:}  The contract-guarded compression
        preserves enough development events around $e_{41}$ to maintain
  ...
  \item \textbf{Naive, $M = 10$:}  The enumerative solver discovers
        that $e_{41}$ is infeasible and substitutes $e_{45}$.  The free
        solve succeeds (valid output), but the fixed-pivot solve fails
        (cannot use $e_{41}$), and pivot preservation is
  ...
  \item \textbf{Contract, $M = 10$:}  Contract-guarded compression
        ...
        not need to search alternatives because $e_{41}$ remains
...
eviction in transformer models.  The model ``knows'' the information
...
Header compliance drops from $0.917$ to $0.417$ at 30\% retention
but then recovers to $0.667$ at 10\%.  This non-monotonicity
suggests that at extreme compression levels, the model defaults to a
formulaic output pattern that happens to include headers---a form of
protocol compliance without semantic content.
...
8B, Mistral~7B, Gemma~2 9B, Phi-3 Medium 14B, and Qwen~2.5 14B.  Each
