...
Lorien narrative simulation---and a specific set of failures that resisted
...
failure: it is a structural guard against Goodhart's law, in which a
measure that becomes a target ceases to be a good measure.  When $Q$ is
the only objective, the search exploits $Q$ at the expense of narrative
structure.  When the grammar co-constrains the search, the search cannot
exploit $Q$ without also satisfying structural requirements.
...
We measure two metrics as a function of \emph{coalition size}~$k$, the
...
simulations.  But VA drops from its $k = 0$ value of 0.652 to a minimum
of 0.617 at $k = 3$ before partially recovering.  The all-valid rate
drops from 88\% at $k = 0$ (no evolution) to 64\% at $k = 6$ (full
evolution).
