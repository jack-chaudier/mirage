...
\paragraph{Paper~00: Narrative / Lorien (Chapters~1--2).}
...
\paragraph{Paper~01: Absorbing States (Chapters~3--4).}
...
types (pool starvation, temporal clustering, weight--position
antagonism, and grammar--constraint conflict) and constructed a
...
\paragraph{Paper~02: Streaming (Chapters~6, 8).}
...
\paragraph{Paper~03: Context Algebra (Chapters~5, 7, 9).}
...
MAP-Elites (multi-dimensional archive of phenotypic elites) offers a
principled alternative.  The behaviour space is discretised into bins
...
StreamingLLM's initial-token retention) is suggestive: the ``sink''
...
Retrieval-augmented generation selects chunks from a document store to
...
own output faces the same structural coupling.  This section sketches
...
Process mining discovers workflow models from event logs.  A central
...
Grammar-constrained decoding systems---such as PICARD (for SQL),
Outlines (for structured JSON), and constrained beam search---guide
