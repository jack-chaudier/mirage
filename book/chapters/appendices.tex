...
\noindent
\begin{small}
\begin{tabular}{@{} p{2.6cm} p{1.4cm} p{1.4cm} p{3.8cm} p{3.0cm} @{}}
\toprule
\textbf{Claim} & \textbf{Test} & \textbf{Paper Exp} & \textbf{Artifact CSV} & \textbf{Figure} \\
\midrule

Tropical exactness (0~violations)
  & \texttt{test\_01}
  & P01 Exp~1
  & \url{results/raw/test_01_exactness.csv}
  & \url{test_01_exactness_heatmap.png} \\

Associativity (0~violations)
  & \texttt{test\_02}
  & P01 Exp~2
  & \url{results/raw/test_02_associativity.csv}
  & \url{test_02_associativity_heatmap.png} \\

Monoid subsumption
  & \texttt{test\_03}
  & P01 Exp~3
  & \url{results/raw/test_03_monoid_subsumption.csv}
  & \url{test_03_monoid_richness.png} \\

Absorbing ideal (0~violations)
  & \texttt{test\_04}
  & P01 Exp~4
  & \url{results/raw/test_04_absorbing_ideal.csv}
  & \url{test_04_absorption_escape_rates.png} \\

Holographic exactness
  & \texttt{test\_05}
  & P01 Exp~5
  & \url{results/raw/test_05_holographic_exactness.csv}
  & --- \\

Incremental consistency
  & \texttt{test\_06}
  & P01 Exp~6
  & \url{results/raw/test_06_incremental_consistency.csv}
  & --- \\

Scaling $O(n \log n)$
  & \texttt{test\_07}
  & P01 Exp~7
  & \url{results/raw/test_07_scaling.csv}
  & \url{test_07_scaling_loglog.png} \\

Divergence (super-linear)
  & \texttt{test\_08}
  & P01 Exp~8
  & \url{results/raw/test_08_divergence_raw.csv}
  & \url{test_08_divergence_loglog.png} \\

Record process
  & \texttt{test\_09}
  & P02 Exp~1
  & \url{results/raw/test_09_record_process_raw.csv}
  & \url{test_09_records_and_min_gap.png} \\

Tropical shield
  & \texttt{test\_10}
  & P02 Exp~2
  & \url{results/raw/test_10_tropical_shield_raw.csv}
  & \url{test_10_tropical_shield_policy_compare.png} \\

Compression mirage
  & \texttt{test\_11}
  & P03 Exp~1
  & \url{results/raw/test_11_mirage_raw.csv}
  & \url{test_11_validity_mirage.png} \\

Deterministic witness
  & \texttt{test\_12}
  & P03 Exp~2
  & \url{results/raw/test_12_deterministic_witness.csv}
  & \url{test_12_deterministic_witness.png} \\

Contract compression
  & \texttt{test\_13}
  & P03 Exp~3
  & \url{results/raw/test_13_contract_compression_raw.csv}
  & \url{test_13_contract_vs_naive.png} \\

Margin correlation
  & \texttt{test\_14}
  & P03 Exp~4
  & \url{results/raw/test_14_margin_correlation_raw.csv}
  & \url{test_14_margin_quartiles.png} \\

Adaptive compression
  & \texttt{test\_15}
  & P03 Exp~5
  & \url{results/raw/test_15_adaptive_compression_raw.csv}
  & \url{test_15_adaptive_vs_uniform.png} \\

Organic traps (54.9\%)
  & \texttt{test\_16}
  & P02 Exp~3
  & \url{results/raw/test_16_organic_traps.csv}
  & \url{test_16_trap_rate_heatmaps.png} \\

Tropical streaming
  & \texttt{test\_17}
  & P02 Exp~4
  & \url{results/raw/test_17_tropical_streaming.csv}
  & \url{test_17_policy_validity_bar.png} \\

Transition depth
  & \texttt{test\_18}
  & P03 Exp~6
  & \url{results/raw/test_18_transition_vector_raw.csv}
  & \url{test_18_transition_vector_depth.png} \\

5-model blackbox
  & ---
  & P03 Ext
  & \url{release/results/blackbox_bf16_5model/*.csv}
  & --- \\

KV-cache eviction
  & ---
  & P03 Ext
  & \url{release/results/kv_cache_eviction_llama31_8b/*.csv}
  & --- \\

NTSB incidents
  & ---
  & P03 Ext
  & \url{results/ntsb/*.csv}
  & --- \\
\bottomrule
\end{tabular}
\end{small}
