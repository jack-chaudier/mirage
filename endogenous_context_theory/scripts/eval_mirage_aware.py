...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--adapter-path",
        default=str(ROOT / "adapters" / "mirage_aware_v1"),
        help="Path to LoRA adapter directory.",
    )
    parser.add_argument(
        "--valid-jsonl",
        default=str(ROOT / "data" / "processed" / "valid.jsonl"),
        help="Validation JSONL with embedded oracle metadata.",
    )
    parser.add_argument(
        "--out-csv",
        default=str(ROOT / "results" / "mirage_aware_eval_results.csv"),
        help="Path for per-example result CSV.",
    )
    parser.add_argument(
        "--summary-csv",
        default=str(ROOT / "results" / "mirage_aware_eval_summary.csv"),
