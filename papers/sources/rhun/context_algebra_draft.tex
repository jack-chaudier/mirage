...
For adjacent blocks $A$ then $B$, define $C_{A\Vert B}=C_A\otimes C_B$:
\[
\begin{aligned}
...
All cases agree; therefore $\otimes$ is associative.
\end{proof}
...
Fix grammar parameter $k$ and define absorbing predicate $\bot$ by $d_{\mathrm{pre}}<k$.

\begin{proposition}[Absorbing left ideal under committed semantics]
If context $A$ has committed pivot and $A\in\bot$, then for any suffix $D$, $A\otimes D\in\bot$.
\end{proposition}
\begin{proof}
Under committed semantics, suffix events cannot replace the committed pivot. Hence
\[
d_{\mathrm{pre},A\otimes D}=d_{\mathrm{pre},A}<k,
\]
so $A\otimes D\in\bot$ for all $D$.
\end{proof}

\begin{remark}[Semantics qualifier]
Without commitment, suffix pivot substitution can escape absorption: if $w^\star_D>w^\star_A$, then $d_{\mathrm{pre}}=d_{\mathrm{total},A}+d_{\mathrm{pre},D}$ may exceed $k$. Exp58 is an explicit witness of this distinction.
\end{remark}
...
Exp58 is a deterministic witness, not a distributional guarantee. Real-incident validation is still small-scale (12 incidents, single-model backend), while most sweeps remain synthetic. A full symbolic characterization of compression-safe contracts beyond the threshold form above remains open.
