...
In practice, stream lengths are often known or bounded (e.g., batch sizes, session limits, or context-window caps), which makes this semi-online assumption operationally reasonable in deployed systems.
...
\begin{proposition}[Record-gap scaling in an i.i.d.\ reference model]
Under i.i.d.\ continuous weights, the minimum gap between consecutive records is $O(1)$ as sequence length grows. In particular, the first inter-record gap has $\Pr(\text{gap}=1)=1/2$.
\end{proposition}
\paragraph{Proof sketch.}
...
\paragraph{Clarification for our synthetic regime.}
Our Exp45 generators are bursty/front-loaded and therefore not literally i.i.d. The proposition above is a reference-model result that explains why constant-scale minimum gaps are plausible. In this paper's generator family, the scale claim is empirical: trap rates stay in the same broad band across tested $n$, and mean minimum gap remains near 1.0.
...
\begin{enumerate}[leftmargin=1.2em]
  \item Jack Chaudier Gaffney. \emph{Absorbing States in Greedy Search: When Endogenous Constraints Break Sequential Extraction}. arXiv preprint, submitted concurrently, 2026.
  \item B. Korte and L. Lov\'asz. Mathematical structures underlying greedy algorithms. In \emph{Fundamentals of Computation Theory (FCT'81)}, 1981.
  ...
  \item B. T. Willard and R. Louf. Efficient guided generation for large language models. \emph{arXiv preprint arXiv:2307.09702}, 2023.
...
\begin{itemize}[leftmargin=1.2em]
  \item \textbf{H1 (superseded):} ``Deferred at $f=0.10$ is near-optimal.'' This came from a scoring-path inconsistency in an earlier evaluation path; corrected TP-consistent analyses show a broader tradeoff frontier.
