...
An event graph is $G=(V,t,w,a)$ where $V$ is a finite event set, $t:V\to\mathbb{R}$ is a strict temporal order, $w:V\to\mathbb{R}_{>0}$ is a weight map, and $a:V\to A$ assigns actors.
\end{definition}
...
Given subsequence $S$ and designated turning point $\tau\in S$, classifier $\phi_\tau:S\to\Sigma$ assigns pre-$\tau$ events to SETUP/DEVELOPMENT (position-based rule), $\tau$ to TURNING\_POINT, and post-$\tau$ events to RESOLUTION.
\end{definition}
...
Given $G$, focal actor $a^*$, grammar parameter $k$, length limit $L$, minimum timespan fraction $s$, and maximum temporal gap $G_{\max}$, find
\[
...
Sort by decreasing weight, inject the max-weight focal event into each candidate pool, and construct a sequence by forward weight-maximizing selection.
\end{definition}
...
Let greedy force $e^*=\arg\max_{v:a(v)=a^*}w(v)$ as TURNING\_POINT. Let grammar require $k\ge 1$ DEVELOPMENT events before TURNING\_POINT. Let $j_{dev}$ be the number of development-eligible events (any actor) with timestamp strictly before $e^*$ in the candidate pool. If $j_{dev}<k$, greedy produces zero valid sequences.
\end{theorem}

\begin{proof}
Use product automaton $\mathcal{P}=G\times\mathcal{A}$.
(1) Injection forces $e^*$ as TP candidate by construction.
...
At $M=25$, recovery is 94\% (multi-burst+gap) and 96.0\% (bursty+gap), with zero regressions versus BVAG. Residuals are diagnosed as either TP-candidate truncation (3 cases) or pool-construction boundary (5 bursty cases), not DP incompleteness.
...
$\tau(P) = \max_{v \in P} t(v) - \min_{v \in P} t(v)$ and the required span
$s \cdot T$ where $T = \max_{v \in V} t(v) - \min_{v \in V} t(v)$. If
$\tau(P) < s \cdot T$, then no sequence $S \subseteq P$ can satisfy the
minimum timespan constraint, regardless of search strategy.
\end{proposition}
\begin{proof}
For any $S \subseteq P$, $\mathrm{span}(S) \leq \tau(P) < s \cdot T$.
\end{proof}
...
Hypothesis & Result \\
\midrule
Shear determines contamination & Rejected ($r=0.033$, 0/9 fixed). \\
